...
ARG jdkLinuxComponentSha256SUM

RUN apt-get update && \
    apt-get install -y --no-install-recommends curl ca-certificates fontconfig unzip locales && \
...
RUN set -eux; \
    ...
    echo "${jdkLinuxComponentSha256SUM} */tmp/openjdk.tar.gz" | sha256sum -c -; \
